...
int tester_resource_write1(char* test_name){
    ...
    TESTER_UTEST_CHECK(tester_utest_assert_int(DSOS_EBADFD, return_value, "Error, disastrOS_write unrecognize non-existent descriptor"));
...
int tester_resource_write3(char* test_name){
    ...
    return_value = disastrOS_write(file_descriptor, buffer, count);
    TESTER_UTEST_CHECK(tester_utest_assert_int(DSOS_ENOSYS, return_value, "Error, the write operation does not indicate that it is not implemented"));
...
int tester_resource_write5_aux(int fd, const void* buffer, int count){return count;}
