...
const char* tester_itest_fifo_example_text =
...
typedef struct Arguments_fifo_text_writer{int resource_id; int text_len; const char* text_buffer;} Arguments_fifo_text_writer;
void test_itest_fifo_text_writer(void* arguments){
    // 0. Retrieve arguments
    int resource_id = ((Arguments_fifo_text_writer*) arguments)->resource_id;
    int text_len = ((Arguments_fifo_text_writer*) arguments)->text_len;
    const char* text_buffer = ((Arguments_fifo_text_writer*) arguments)->text_buffer;
    // 1. Open fifo in write only mode
    printf(TESTER_WRITER_TAG "startup\n");
    int fd = disastrOS_open(resource_id, DSOS_O_WRONLY);
    if(fd<0){
        printf(TESTER_WRITER_TAG "disastrOS_open returned %d\n", fd);
        disastrOS_exit(-1);
    }
    // 2. Write on the fifo
    int writted = disastrOS_write(fd, text_buffer, text_len);
    printf(TESTER_WRITER_TAG "ended\n");
    // 3. Close descriptor and return number of written byte
    int return_value = disastrOS_close(fd);
...
typedef struct Arguments_fifo_text_reader{int resource_id; int text_len; char* text_buffer;} Arguments_fifo_text_reader;
void test_itest_fifo_text_reader(void* arguments){
    // 0. Retrieve arguments
    int resource_id = ((Arguments_fifo_text_reader*) arguments)->resource_id;
    int text_len = ((Arguments_fifo_text_reader*) arguments)->text_len;
    char* text_buffer = ((Arguments_fifo_text_reader*) arguments)->text_buffer;
    // 1. Open fifo in read only mode
    printf(TESTER_READER_TAG "startup\n");
    int fd = disastrOS_open(resource_id, DSOS_O_RDONLY);
    if(fd<0){
            printf(TESTER_READER_TAG "disastrOS_open returned %d\n", fd);
            disastrOS_exit(-1);
    }
    // 2. Read from the fifo
    int return_value, readed = 0;
    while(1){
        return_value = disastrOS_read(fd, text_buffer+readed, text_len-readed);
...
typedef struct Arguments_fifo_packet_reader{int reader_id; int resource_id; int sleep_time; int sleep_rate;}Arguments_fifo_packet_reader;
void test_itest_fifo_packet_reader(void* arguments){
    ...
    int sleep_time = ((Arguments_fifo_packet_reader*) arguments)->sleep_time;
    int sleep_rate = ((Arguments_fifo_packet_reader*) arguments)->sleep_rate;
    // 1. Open fifo in read only mode
    printf(TESTER_READER_TAG_WID "startup\n", reader_id);
    int fd = disastrOS_open(resource_id, DSOS_O_RDONLY);
    if(fd<0){
            printf(TESTER_READER_TAG_WID "disastrOS_open returned %d\n", reader_id, fd);
            disastrOS_exit(-1);
    }
    // 2. Read packet from the fifo
    int return_value, received = 0;
    char buffer[7];
    while(1){
    ...
    // 3. Close read file descriptor and return
    printf(TESTER_READER_TAG_WID "ended, total packet received: %d\n", reader_id, received);
    return_value = disastrOS_close(fd);
    if(return_value != DSOS_SUCCESS){
        printf(TESTER_READER_TAG_WID "disastrOS_close (during close WR descriptor at the end) return %d\n", reader_id, return_value);
        disastrOS_exit(-1);
    }
...
typedef struct Arguments_fifo_packet_writer{int writer_id; int resource_id; int to_send; int sleep_time; int sleep_rate;}Arguments_fifo_packet_writer;
void test_itest_fifo_packet_writer(void* arguments){
    ...
    int sleep_time = ((Arguments_fifo_packet_writer*) arguments)->sleep_time;
    int sleep_rate = ((Arguments_fifo_packet_writer*) arguments)->sleep_rate;
    // 1. Open fifo in write only mode
    printf(TESTER_WRITER_TAG_WID "startup\n", writer_id);
    int fd = disastrOS_open(resource_id, DSOS_O_WRONLY);
    if(fd<0){
        printf(TESTER_WRITER_TAG_WID "disastrOS_open returned %d\n", writer_id, fd);
        disastrOS_exit(-1);
    }
    // 2. Write packet on the fifo
    const char buffer[7] = "PACKET";
    int sended = 0, return_value;
    ...
    // 3. Close write file descriptor and return
    printf(TESTER_WRITER_TAG_WID "ended, total packet sended: %d\n", writer_id, sended);
    return_value = disastrOS_close(fd);
    if(return_value != DSOS_SUCCESS){
        printf(TESTER_WRITER_TAG_WID "disastrOS_close (during close WR descriptor at the end) return %d\n", writer_id, return_value);
        disastrOS_exit(-1);
    }
...
void test_itest_fifo1_init(){
    ...
    int return_value = disastrOS_mkfifo(25);
    if(return_value != DSOS_SUCCESS){
        printf(TESTER_INIT_TAG "error, disastrOS_make return %d, expected 0\n", return_value);
        return;
    }
    // 2. Spawn 1 writer (producer)
    Arguments_fifo_text_writer writer_arguments = {
        .resource_id = 25,
        .text_buffer = tester_itest_fifo_example_text,
        .text_len= strlen(tester_itest_fifo_example_text),
    };
    int writer_pid = last_pid;
    disastrOS_spawn(test_itest_fifo_text_writer, &writer_arguments);
    printf(TESTER_INIT_TAG "spawned writer\n");
    // 3. Spawn 1 reader (consumer)
    char readed_text[strlen(tester_itest_fifo_example_text)];
    Arguments_fifo_text_reader reader_arguments = {
        .resource_id = 25,
        .text_buffer = readed_text,
        .text_len = strlen(tester_itest_fifo_example_text),
    };
    int reader_pid = last_pid;
    disastrOS_spawn(test_itest_fifo_text_reader, &reader_arguments);
...
void test_itest_fifo2_init(){
    ...
    int readers_pid[10];
    Arguments_fifo_packet_reader readers_arguments[10];
    for(int i=0; i<10; i++){
        readers_arguments[i].resource_id = 25,
        readers_arguments[i].reader_id = i+1;
        readers_arguments[i].sleep_time = 4;
        readers_arguments[i].sleep_rate = 5;
        readers_pid[i] = last_pid;
        disastrOS_spawn_withfd(test_itest_fifo_packet_reader, &readers_arguments[i]);
...
void test_itest_fifo3_init(){
    ...
    int writers_pid[10];
    Arguments_fifo_packet_writer writers_arguments[10];
    for(int i=0; i<10; i++){
        writers_arguments[i].resource_id = 25;
        ...
        disastrOS_spawn(test_itest_fifo_packet_writer, &writers_arguments[i]);
        printf(TESTER_INIT_TAG "spawned writer: %d\n", i+1);
    }
    // 3. Spawn 1 reader (consumer)
    Arguments_fifo_packet_reader reader_arguments = {
        .resource_id = 25,
        .reader_id = 1,
        .sleep_time = 4,
        .sleep_rate = 5,
    ...
    int reader_pid = last_pid;
    disastrOS_spawn(test_itest_fifo_packet_reader, &reader_arguments);
    printf(TESTER_INIT_TAG "spawned reader\n");
    // 5. Wait for writers
    int pid;
    int writed = 0;
...
void test_itest_fifo4_init(){
    ...
    for(int i=0; i<10; i++){
        ...
        readers_arguments[i].sleep_rate = 5;
        readers_pid[i] = last_pid;
        disastrOS_spawn(test_itest_fifo_packet_reader, &readers_arguments[i]);
        printf(TESTER_INIT_TAG "spawned reader: %d\n", i+1);
    }
    // 5. Wait for writers
    int pid, writed = 0;
