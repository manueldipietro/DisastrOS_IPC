...
int tester_utest_ipc_read1(char* test_name){
    ...
    TESTER_UTEST_CHECK(tester_utest_assert_ecode(count, return_value, "error on ipc read"));
    
    // 4. Check for attribute of ipc after read
    TESTER_UTEST_CHECK(tester_utest_assert_int(size_max-count, ipc->size, "mismatch on ipc size attribute"));
    TESTER_UTEST_CHECK(tester_utest_assert_int(size_max, ipc->size_max, "mismatch on ipc size_max attribute"));
...
int tester_utest_ipc_read2(char* test_name){
    ...
    ipc = (Ipc*) ResourceList_byId(&resources_list, resource_id);
    TESTER_UTEST_CHECK(tester_utest_assert_allocated(ipc, "can't retrieve ipc from list"));
    TESTER_UTEST_IPC_ASSERT_READERS_WAITING(ipc, 1, reader_pid, "After read");
    TESTER_UTEST_CHECK(tester_utest_assert_int(0, ipc->size, "mismatch on ipc size attribute after read"));
...
int tester_utest_ipc_read3(char* test_name){
    ...
    TESTER_UTEST_CHECK(tester_utest_assert_ecode(count, return_value, "error on ipc write"));
    disastrOS_sleep(1);

    // 5. Check that reader unblock and check ipc size
    TESTER_UTEST_IPC_ASSERT_READERS_UNLOCKANDEXITANDWAIT(ipc, 0, reader_pid, "after write");
    TESTER_UTEST_CHECK(tester_utest_assert_int(0, ipc->size, "mismatch on ipc size attribute after unblock read"));
...
int tester_utest_ipc_read4(char* test_name){
    ...
    ipc = (Ipc*) ResourceList_byId(&resources_list, resource_id);
    TESTER_UTEST_CHECK(tester_utest_assert_allocated(ipc, "can't retrieve ipc from list"));
    TESTER_UTEST_IPC_ASSERT_READERS_WAITING(ipc, 2, reader1_pid, "after read");
    TESTER_UTEST_IPC_ASSERT_READERS_WAITING(ipc, 2, reader2_pid, "after read");
    TESTER_UTEST_CHECK(tester_utest_assert_int(0, ipc->size, "mismatch on ipc size attribute after read"));
    ...
    // 5. Check that reader unblock and check ipc size
    TESTER_UTEST_IPC_ASSERT_READERS_UNLOCKANDEXITANDWAIT(ipc, 0, reader1_pid, "first read unlock");
    TESTER_UTEST_IPC_ASSERT_READERS_UNLOCKANDEXITANDWAIT(ipc, 0, reader2_pid, "second reader unlock");
    TESTER_UTEST_CHECK(tester_utest_assert_int(1, ipc->size, "mismatch on ipc size attribute after unblock read"));
...
int tester_utest_ipc_read6(char* test_name){
    ...
    TESTER_UTEST_IPC_ASSERT_WRITERS_WAITING(ipc, 1, writer_pid, "after read");
    TESTER_UTEST_CHECK(tester_utest_assert_int(15, ipc->size, "mismatch on ipc size attribute after write"));
    ...
    TESTER_UTEST_IPC_ASSERT_WRITERS_UNLOCKANDEXITANDWAIT(ipc, 0, writer_pid, "after read");
    TESTER_UTEST_CHECK(tester_utest_assert_int(15, ipc->size, "mismatch on ipc size attribute after unblock write"));
...
int tester_utest_ipc_read7(char* test_name){
    ...
    // 3. Read from buffer
    return_value = disastrOS_read(file_descriptor, buffer, count);
    TESTER_UTEST_CHECK(tester_utest_assert_ecode(7, return_value, "error on ipc read"));
    
    // 4. Check for attribute of ipc after read
    TESTER_UTEST_CHECK(tester_utest_assert_int(0, ipc->size, "mismatch on ipc size attribute"));
    TESTER_UTEST_CHECK(tester_utest_assert_listsize(&ipc->waiting_list_read, 0, "unexpected waiting process in ipc waiting_list_read"));
...
int tester_utest_ipc_read8(char* test_name){
    ...
    TESTER_UTEST_CHECK(tester_utest_assert_allocated(ipc, "can't retrieve ipc from list"));
    TESTER_UTEST_CHECK(tester_utest_assert_int(0, ipc->size, "mismatch on ipc size attribute after read"));
    TESTER_UTEST_CHECK(tester_utest_assert_listsize(&ipc->waiting_list_read, 0, "unexpected waiting process in ipc waiting_list_read after read"));
...
int tester_utest_ipc_read10(char* test_name){
    ...
    return_value = disastrOS_read(file_descriptor, buffer, count);
    TESTER_UTEST_CHECK(tester_utest_assert_ecode(0, return_value, "error on read with count = 0"));
    TESTER_UTEST_CHECK(tester_utest_assert_int(0, ipc->size, "mismatch on ipc size attribute after read with count = 0"));
    TESTER_UTEST_CHECK(tester_utest_assert_listsize(&ipc->waiting_list_read, 1, "unexpected waiting process in ipc waiting_list_read after read with count = 0"));
